...
COMPOSE_SERVICE_SOURCE_PATH="${HOME}/.config/systemd/user/${COMPOSE_SERVICE_NAME}"
...
main() {
  ...
  copy_if_exists "${COMPOSE_SERVICE_SOURCE_PATH}" "${BACKUP_DIR}/${COMPOSE_SERVICE_NAME}"
  copy_if_exists "${INTERRUPT_DIR}/deploy/compose/env.robot" "${BACKUP_DIR}/env.robot"
