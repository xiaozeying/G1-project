...
from src.safe_action_gateway import evaluate_action_safety
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a structured camera observation and evaluate whether a body action is safe."
    )
    parser.add_argument("action", nargs="?", default="high wave")
    parser.add_argument(
        "--question",
        default="请检查前方空间、人体接近情况，以及手臂活动范围附近是否有遮挡物。",
    )
    parser.add_argument(
        "--language",
        default="zh-CN",
...
def main() -> int:
    ...
    if not result.ok:
        ...
        return 1
    decision = evaluate_action_safety(args.action, result.observation)
    print(
        json.dumps(
            {
                "ok": True,
